...
> **최종 업데이트**: 2026-03-08 (Token Diet + 컨텍스트 윈도우 재측정)
...
|----------|------|------|
| **시스템 프롬프트** | | |
| ├ Core theories (5개) | ~4,200 | IDENTITY+LENSES(4종) |
| ├ Rule tables (14개 상시) | ~5,360 | STATE_TRACK~ITEM_AWARE |
| ├ Conditional rules (~1개 평균) | ~1,100 | SEXUAL_PSYCH 또는 FLASHBACK |
| ├ Output schema | ~3,750 | 전체 DAI JSON 스키마 |
| ├ Theory emphasis + spotlight | ~1,570 | GENRE_WEIGHTS+modules |
| └ Module absent guide | ~170 | |
| **시스템 프롬프트 소계** | **~16,150** | |
| **유저 메시지** | | |
...
| ├ 정적 슬롯 (text_resources) | **~8,300** | ★ was 14,135 → diet -5,835 |
| ├ Slot 6: PC 데이터 | ~430 | |
| ├ Slot 7: NPC 5명 장면별 + 보이스카드 + 14명 | ~10,400 | |
| ├ Slot 8: 로어 청크 5개 | ~4,500 | |
| ├ Slot 9: 발효 기억 | ~600 | |
| ├ Slot 11: 챕터 컨텍스트 | ~300 | ★ 이전 누락 |
| ├ Slot 13-17: DAI 아이스버그+심리+장면+대사지시 | ~1,210 | |
| ├ Slot 20: 상태 윈도우 | ~150 | ★ 이전 누락 |
| ├ Slot 22-24: 콘텐츠 수위 | ~200 | ★ 이전 누락 |
| ├ Slot 27: 시간 우선순위 | ~100 | ★ 이전 누락 |
| ├ Slot 28: 서사 체인 + 페이싱 | ~400 | ★ 이전 누락 |
| ├ Slot 29: 실시간 데이터 | ~300 | ★ 이전 누락 |
| ├ Slot 30: GM 무버 + 인지통합 | ~400 | ★ 이전 누락 |
| ├ Slot 31-33: 직전응답+입력+노트 | ~500 | ★ 이전 누락 |
| └ 초기화 래퍼 | ~50 | |
| **Message 1 소계** | **~27,840** | |
| **Message 2-4: 더미 + 트레이닝** | ~200 | |
| **Message 5+: smart_history** | **~10,000** | ★ was 4,200 → 정확 측정 |
| **현재 사용자 메시지 + 프리필** | ~250 | |
| **입력 합계** | **~38,290** | |
...
#### 정적 슬롯 내역 (Token Diet 후 실측)

| 슬롯 | 내용 | 글자 수 | 토큰 |
|------|------|---------|------|
| 1 | MANDATE (CONTENT_AUTHORIZATION) | 3,101 | ~886 |
| 2 | AI_CORE_IDENTITY | 1,316 | ~376 |
| 3 | MIRROR_WORKSHOP | 6,003 | ~1,715 |
| 4 | PHYSICAL_RENDERING | 1,480 | ~423 |
| 5 | WORLD_AXIOM + MEMORY_HIERARCHY | 1,320 | ~377 |
| 10 | TEMPORAL_FLOW + TIME_ATMOSPHERE | 1,498 | ~428 |
| 12 | INTERACTION + NPC_BEHAVIOR | 3,155 | ~901 |
| 15 | PSYCHE_STATE_RENDERING | 354 | ~101 |
| 18 | PC_AUTONOMY | 1,077 | ~308 |
| 21 | ACTION + ASPECT + SITUATION | 1,152 | ~329 |
| 25 | ANTI_CLICHE + PROSE_CRAFT | 4,600 | ~1,314 |
| 28* | PACING_CONTROL | 475 | ~136 |
| 30* | COGNITIVE_DATA_INTEGRATION | 458 | ~131 |
| 34 | TELESCOPE + LANGUAGE_CORRECTION | 2,960 | ~846 |
| | **합계** | **28,949** | **~8,271** |

> *28, 30은 동적 슬롯에 삽입되는 상시 상수
...
```
Flash 입력: 27,568 × $0.50 / 1,000,000 = $0.01378
...
```
Pro 입력: 38,290 × $2.00 / 1,000,000 = $0.07658
...
```
$0.02136 (Flash) + $0.09410 (Pro) = $0.11546 / turn
                                   ≈ ₩173 / turn (환율 ₩1,500)
```
...
```
구분          이전(02-27)  보정치(실제)   현재(03-08)
정적 슬롯      14,135      14,135        8,300       ← diet -5,835
동적 슬롯      17,590      19,490        19,490      ← 누락분 +1,900 발견
히스토리        4,200      10,000        10,000      ← smart_history 정확 측정
기타              600         600           500
──────────────────────────────────────────────────
Pro 입력합    36,525      44,225        38,290
턴당 비용     $0.1116     $0.1265       $0.1155
```

> **핵심**: 이전 계산은 히스토리(4,200)와 동적 슬롯 일부를 누락하여 실제보다 ~$0.015 싸게 표기됨.
> Token Diet로 정적 슬롯 -5,835 tok 절감하여 보정 후 실비용 대비 ~$0.011/turn 절감 달성.
...
|--------|-------|-----|---------------|---------------|
| **100** | $2.14 | $9.41 | **$11.55** | **₩17,300** |
| **200** | $4.27 | $18.82 | **$23.09** | **₩34,600** |
| **300** | $6.41 | $28.23 | **$34.64** | **₩52,000** |
| **400** | $8.54 | $37.64 | **$46.18** | **₩69,300** |
| **500** | $10.68 | $47.05 | **$57.73** | **₩86,600** |
...
```
전체 비용 중:
├── Pro 입력 (Main 프롬프트+히스토리) 66.3%  ← 최대 비용 드라이버
│     ├── 정적 슬롯 (text_resources)  14.4%
│     ├── NPC 프로필 (Slot 7)         18.0%  ← 정적 다음으로 큰 단일 항목
│     ├── 로어 (Slot 8)               7.8%
│     ├── 히스토리 (smart_history)    17.3%  ← 이전 누락, 실제 비중 높음
│     └── 기타 동적 슬롯              8.8%
├── Pro 출력 (서사+텔레스코프)        15.2%
├── Flash 입력 (Theoria+추출)        12.5%
├── Flash 출력 (DAI+추출 JSON)        5.2%
└── 발효 상각                         0.8%
```

**최적화 우선순위**: NPC 프로필(18%) ≈ 히스토리(17%) > 정적 슬롯(14%, diet 완료) > Pro 출력(15%) > Flash(13%)
...
|--------|------------|------------|--------|
| **100** | $9.41 | ₩14,100 | 18.5% |
| **200** | $18.82 | ₩28,200 | 18.5% |
| **300** | $28.23 | ₩42,300 | 18.5% |
| **400** | $37.64 | ₩56,500 | 18.5% |
| **500** | $47.05 | ₩70,600 | 18.5% |
...
| **2026-03-08** | **계산서 보정 (히스토리+동적 슬롯 누락분 반영)** | — | — |
| | | **Diet 순절감 (보정 후 기준)** | **~$0.011** |
...
Flash입력토큰 = Theoria시스템(16,150) + Theoria유저(8,433) + Physical(1,300) + Batch(1,505)
Flash출력토큰 = DAI(2,000) + Physical(85) + Batch(433)

Pro입력토큰 = [Message 1: 34-Slot Prompt]
            + 정적슬롯(8,300) + PC(430) + NPC슬롯7(변동) + 로어슬롯8(변동)
            + 발효(600) + 챕터(300) + DAI아이스버그(1,210) + 상태윈도우(150) + 콘텐츠(200)
            + Slot27-30(1,200) + Slot31-33(500) + 초기화(50)
            + [Message 2-4: 더미+트레이닝](200)
...
> ⚠️ 위 계산은 `3.5자/토큰` (한영 혼합 보수적) 기준. 실제 Gemini 과금은 SentencePiece 토크나이저 기반.
...
- **Context Caching**: Gemini 캐시 사용 시 정적 슬롯(~8,300토큰) 재전송 비용 절감 가능 (현재 미사용)
