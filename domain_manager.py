...
def initialize_folders() -> None:
    ...
    folders = [SESSIONS_DIR, LORE_DIR, RULES_DIR]
...
def get_scene_type(channel_id: str) -> str:
    ...
    Returns:
        'normal', 'gore', 'nsfw', 'gore_nsfw' 중 하나
    """
...
def is_mature_scene(channel_id: str) -> bool:
    """
    현재 장면이 성인 묘사가 활성화된 장면인지 확인합니다.
    
    Returns:
        True if scene_type is 'gore', 'nsfw', or 'gore_nsfw'
    """
    scene_type = get_scene_type(channel_id)
    return scene_type in ['gore', 'nsfw', 'gore_nsfw']


# =========================================================
# 비일상 감지 설정 (Abnormal Elements Detection)
# =========================================================
def is_abnormal_detection_enabled(channel_id: str) -> bool:
    """
    비일상 요소 감지가 활성화되어 있는지 확인합니다.
    기본값: False (비활성화) - 세션 시작 시 꺼져 있으며, !비일상 켜기로 활성화
    """
    return get_domain(channel_id).get("abnormal_detection", False)


def set_abnormal_detection(channel_id: str, enabled: bool) -> None:
    """
    비일상 요소 감지를 활성화/비활성화합니다.
    
    Args:
        enabled: True면 활성화, False면 비활성화
    """
    d = get_domain(channel_id)
    d["abnormal_detection"] = enabled
    save_domain(channel_id, d)


# =========================================================
# 비일상 발생 카운터 (Abnormal Event Trigger System)
# 시간/장소 변화에 따라 천천히 증가, 100이 되면 0.1% 확률로 발생
# =========================================================
def get_abnormal_trigger_counter(channel_id: str) -> int:
    """
    비일상 발생 카운터를 반환합니다. (0-100)
    """
    return get_domain(channel_id).get("abnormal_trigger_counter", 0)


def set_abnormal_trigger_counter(channel_id: str, value: int) -> None:
    """
    비일상 발생 카운터를 설정합니다.
    """
    d = get_domain(channel_id)
    d["abnormal_trigger_counter"] = max(0, min(100, value))
    save_domain(channel_id, d)


def increment_abnormal_trigger(channel_id: str, amount: int = 1) -> int:
    """
    비일상 발생 카운터를 증가시키고 현재 값을 반환합니다.
    ...
    """
    d = get_domain(channel_id)
    current = d.get("abnormal_trigger_counter", 0)
    new_value = min(100, current + amount)
    d["abnormal_trigger_counter"] = new_value
    save_domain(channel_id, d)
    return new_value


def reset_abnormal_trigger(channel_id: str) -> None:
    """
    비일상 발생 카운터를 0으로 리셋합니다.
    """
    d = get_domain(channel_id)
    d["abnormal_trigger_counter"] = 0
    save_domain(channel_id, d)


def get_last_location(channel_id: str) -> str:
    """
    마지막 위치를 반환합니다.
    """
    return get_domain(channel_id).get("last_location", "")


def set_last_location(channel_id: str, location: str) -> None:
    """
    마지막 위치를 설정합니다.
    """
    d = get_domain(channel_id)
...
def get_last_time(channel_id: str) -> str:
    """
    마지막 시간을 반환합니다.
    """
    return get_domain(channel_id).get("last_time", "")


def set_last_time(channel_id: str, time_str: str) -> None:
    """
    마지막 시간을 설정합니다.
    """
    d = get_domain(channel_id)
    d["last_time"] = time_str
    save_domain(channel_id, d)


def check_abnormal_trigger_conditions(channel_id: str, user_id: str) -> bool:
    """
    비일상 이벤트 발생 조건을 확인합니다.
    
    조건:
    1. 비일상 감지가 활성화되어 있어야 함
    2. 아무런 비일상이 존재하지 않거나, 모든 비일상 적응도가 80% 이상이어야 함
    3. gore/nsfw 장면이 아니어야 함
    
    Returns:
        True면 비일상 이벤트 발생 가능
    """
    # 비일상 감지가 비활성화되어 있으면 발생 불가
    if not is_abnormal_detection_enabled(channel_id):
        return False
    
    # gore/nsfw 장면이면 발생 불가
    scene_type = get_scene_type(channel_id)
    if scene_type in ['gore', 'nsfw', 'gore_nsfw']:
        return False
    ...
    # 모든 비일상 적응도가 80% 이상인지 확인
    for element, status in normalization.items():
        level = adaptation_levels.get(status, 0)
        if level < 80:
            return False
